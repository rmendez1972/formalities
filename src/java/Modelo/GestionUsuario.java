...
public class GestionUsuario {
    ...
    public Usuario obtenerPorId(int id_usuario){
        ...
        try{
            if(res.next())
                usr=new Usuario(res.getInt("id_usuario"), res.getString("usuario"), res.getString("password"), res.getString("nombre"), res.getString("apellido_paterno"), res.getString("apellido_materno"), res.getInt("id_unidadadministrativa"), res.getInt("id_grupo"));
            res.close();
        }catch(Exception e){}
        return usr;
    }
    ...
    public ArrayList obtenerUsuarios(){
        ...
        ResultSet res=Conexion.ejecutarConsulta("select U.*, UA.nombre as unidadAdministrativa, G.nombre as grupo from usuario U inner join unidadadministrativa UA on U.id_unidadadministrativa=UA.id_unidadadministrativa inner join grupo G on U.id_grupo=G.id_grupo order by concat(U.nombre,' ',U.apellido_paterno,' ',U.apellido_materno) asc", null);
        try{
            while(res.next()){
                Usuario usr=new Usuario(res.getInt("id_usuario"), res.getString("usuario"), res.getString("password"), res.getString("nombre"), res.getString("apellido_paterno"), res.getString("apellido_materno"), res.getInt("id_unidadadministrativa"), res.getInt("id_grupo"));
    ...
    public boolean actualizarUsuario(Usuario usr){
        Object params[]={usr.getUsuario(), usr.getNombre(), usr.getApellido_paterno(), usr.getApellido_materno(), usr.getId_unidadadministrativa(), usr.getId_grupo(), usr.getId_usuario()};
        return Conexion.ejecutar("update usuario set usuario=?, nombre=UPPER(?), apellido_paterno=UPPER(?), apellido_materno=UPPER(?), id_unidadadministrativa=?, id_grupo=? where id_usuario=?", params);
    }
