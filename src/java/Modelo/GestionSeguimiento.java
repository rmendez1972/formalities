...
public class GestionSeguimiento {
    ...
    public ArrayList obtenerPorSolicitud(int id_solicitud){
        ...
        ResultSet res=Conexion.ejecutarConsulta("select S.id_seguimiento,S.fecha,S.observaciones,S.id_usuario,S.id_solicitud,S.id_status,E.nombre as estatus,S.adjunto from seguimiento S inner join status E on S.id_status=E.id_status where id_solicitud=? order by fecha desc", params);
        try{
            while(res.next()){
